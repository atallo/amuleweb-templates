 ...
 *
 * app.js -- "flattened": Marcello Zaniboni's flat re-skin of aMule's stock
 * web template, reproduced as a single-page app on top of the shared JSON
 * layer (api.php). Markup, texts and number formats are transcribed from
 * the original server-rendered pages; only the transport changed.
...
// root) and in the dev mock preview (/templates/flattened/).
...
// The classic framed box, flattened-style: the caption became a bold
// left-aligned title above the frame instead of a centered <caption>.
// (Kept outside the table so table-layout:fixed still sizes the 24px
// corner columns from the first row on phones.)
function Box({ caption, children }) {
	return html`
	${caption ? html`<div class="boxtitle"><b>${caption}</b></div>` : ''}
...
function Hdr({ go }) {
	// The flattened template replaced the "exit / log / configuration" text
	// links by three more rollover icons (with tooltips), logout pushed to
	// the far right -- and dropped the col.png separator.
	const nav = [
		...
		['stats', 'nav-stats', 'statistics'],
		['log', 'nav-logs', 'logs'],
		['prefs', 'nav-settings', 'configuration'],
	];
	return html`
	<div class="hdr">
		...
		<div class="band">
			...
			<div class="nav navright">
				<a class="navbtn nav-logout" title="exit" href="login.php"></a>
			</div>
		</div>
	</div>`;
}
...
function SearchView({ data, status, guard, refresh }) {
	...
	// The flattened template reorganized the form into three stacked rows
	// (name+availability / network+sizes / buttons), added a "Refresh
	// results" button and a soft gradient rule before the results.
	return html`
	<${Box} caption="SEARCH">
		<form onSubmit=${doSearch}>
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				<tr>
					<td>
						File name: <input type="text" size="60" value=${q} onInput=${(e) => setQ(e.target.value)} />
						${'  '}
						Availability: <input type="text" size="6" value=${avail} onInput=${(e) => setAvail(e.target.value)} />
					</td>
				</tr>
				<tr>
					<td>
						...
						${'    '}
						Min Size: <input type="text" size="5" value=${minV} onInput=${(e) => setMinV(e.target.value)} />
						${' '}${unitSel(minU, setMinU)}
						${'    '}
						Max Size: <input type="text" size="5" value=${maxV} onInput=${(e) => setMaxV(e.target.value)} />
				...
				<tr>
					<td>
						...
						${' '}
						<input type="button" value="Refresh results" title="Update the search results"
							onClick=${() => refresh()} />
					</td>
				</tr>
				<tr><td style="padding-top:20px;padding-bottom:20px"><hr class="softrule" /></td></tr>
			</table>
		</form>
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
			<tr>
...
// The flattened template trimmed this page down to the bootstrap form
// (the stock Network connect/disconnect buttons and nodes.dat URL update
	...
	<${Box} caption="KADEMLIA">
		<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
			<tr valign="top" class="kadcols">
				...
				<td valign="top">
					<table border="0" align="center" cellpadding="0" cellspacing="6" class="kadnewnode">
